  ...
  id_press(nullptr), temperature(nullptr), pressure(nullptr)
{
  ...
  memory->create(flux_na, 15, "rigid_nh:flux");
  memory->create(flux_oxy, 15, "rigid_nh:flux");
  // open the data file
...
FixRigidNH::~FixRigidNH()
{
  ...
  memory->destroy(flux);
  memory->destroy(flux_na);
  memory->destroy(flux_oxy);
  mydata.close();
...
void FixRigidNH::final_integrate()
{
  ...
  for(int ibody=0; ibody<nbody; ibody++){
    ...
    molid = mol2body[molecule[i]];
    if (molid>256){
        flux_na[0]+=KinEng*vcm[ibody][0];
        flux_na[1]+=KinEng*vcm[ibody][1];
        flux_na[2]+=KinEng*vcm[ibody][2];
        flux_na[3]+=RotEng*vcm[ibody][0];
        flux_na[4]+=RotEng*vcm[ibody][1];
        flux_na[5]+=RotEng*vcm[ibody][2];
        flux_na[6]+=body_pe[ibody]*vcm[ibody][0];
        flux_na[7]+=body_pe[ibody]*vcm[ibody][1];
        flux_na[8]+=body_pe[ibody]*vcm[ibody][2];
    }
    if (molid<257){
        flux_oxy[0]+=KinEng*vcm[ibody][0];
        flux_oxy[1]+=KinEng*vcm[ibody][1];
        flux_oxy[2]+=KinEng*vcm[ibody][2];
        flux_oxy[3]+=RotEng*vcm[ibody][0];
        flux_oxy[4]+=RotEng*vcm[ibody][1];
        flux_oxy[5]+=RotEng*vcm[ibody][2];
        flux_oxy[6]+=body_pe[ibody]*vcm[ibody][0];
        flux_oxy[7]+=body_pe[ibody]*vcm[ibody][1];
        flux_oxy[8]+=body_pe[ibody]*vcm[ibody][2];
    }
...
//     body_properties<< vcm[ibody][0]<<" " << vcm[ibody][1]<<" "<< vcm[ibody][2]<< " ";
//     body_properties<< xcm[ibody][0]<<" " << xcm[ibody][1]<<" "<< xcm[ibody][2]<< " ";
//     body_properties<< angmom[ibody][0]<<" " << angmom[ibody][1]<<" "<< angmom[ibody][2]<< " ";
//     body_properties<< final_torque[0][ibody][0]<<" " << final_torque[0][ibody][1]<<" "<< final_torque[0][ibody][2]<< " ";
//     body_properties<<endl;
//   }
// body_properties<<endl;
