...
class AtomsToGraphs:
    ...
    def convert(self, atoms: ase.Atoms, thermal_conductivity, sid=None):
        ...
        if self.r_energy:
            energy = thermal_conductivity #atoms.get_potential_energy(apply_constraint=False)
            data.y = energy
        if self.r_forces:
    ...
    def convert_all(
        ...
        for cry_ind, atoms in enumerate(tqdm(
            atoms_iter,
            ...
            if not isinstance(atoms, ase.atoms.Atoms):
                atoms = atoms.toatoms()
            data = self.convert(atoms, thermal_conductivity[cry_ind])
